...
## 3. 里程碑计划（M1–M17：主链 20 步 + 扩展 14 步；2.0 段 M18–M19 共 4 步；每里程碑 ≤ 2 步）
...
── 2.0 段（M18–M20，doc/10 表情与情绪系统，用户命名「2.0 计划」）──
M18 常驻表情 + 情绪内核(可跑) → M19 全屏 Robot 互动模式(可跑) → M20 打磨：过渡动画 + 走查调参(可跑，低优先)
（M18 依赖 M10 表现层 Emotion + M5/M7 结算链路 + `grokBot` 库接入；M19 依赖 M18 情绪内核 + L1 顶层手势层重构；M20 仅打磨，不阻塞前两个里程碑的验证）
```
...
**2.0 里程碑（M18–M19，doc/10 表情与情绪系统）**：

| 里程碑 | 主题 | 步数 | 收盘演示 | 依赖 |
|---|---|---|---|---|
| M18 | 常驻 Robot 表情 + 情绪内核 | 2 | 主屏顶部常驻反色表情、情绪随属性/离线结算变化 | M10 Emotion / M5+M7 结算 / `grokBot` 接入 |
| M19 | 全屏 Robot 互动模式 | 2 | 点表情全屏 + 宠物缩底部（静态帧）→ 点缩略回游走；Robot 展示层自接点/滑/长按并涨属性 | M18 情绪内核 / L1 顶层手势层 |
| M20 | 打磨（低优先） | 2 | 切换过渡动画（共享元素缩放）+ 常动功耗走查与调参 | M18 / M19 |
...
### 2.0 段：表情与情绪系统（M18–M19）

> 设计依据：**doc/10-表情与情绪系统设计.md**（四层模型、情绪模型、映射表、手势归属、决策记录）。本节只拆步与验收。
> 用户原始诉求（五条）：① 顶部异常标识改为**常驻 Robot 表情**（反色：脸白眼黑，尺寸 = `min(宽,高) × 0.2`）；② 引入**情绪系统**，启动即运行、应用停止即结束，初态 Sleep，离线结算完成后按「离线事件 + 当前属性」落情绪，情绪随属性与交互变化；③ 点顶部 Robot → **全屏 Robot**，宠物缩为固定图标钉在**屏幕底部中央**（与 ① 共享尺寸）；④ 全屏 Robot 支持**触摸交互**（点击/滑动）影响情绪，**边缘手势不受影响**；⑤ 点底部宠物缩略 → 切回**宠物游走模式**。
>
> **分层铁律（用户确认，doc/10 §4）**：**手势跟着面板/组件自己走，收起态天然没有事件**。顶层 L1 = 三向抽屉手势层（边缘带内消费、带外放行）；中层 L2 = OSD 状态展示层（**纯展示，不接手势**）；中下层 L3 = 缩略态控件（Robot 缩略表情 / 宝可梦缩略按钮，只 `clickable` 切模式）；下层 L4 = 展开态内容层（宝可梦游走层、Robot 表情展示层，**Robot 展开时自己接管点/滑/长按**）。Robot 手势用 `requireUnconsumed = true`，边缘事件被 L1 消费后到不了它 —— 天然隔离，不做矩形规避。

### M18 常驻 Robot 表情 + 情绪内核（可跑）

> 目的：先把 `:grokBot` 依赖与主屏常驻表情跑通（依赖/功耗/小屏观感三个风险前置），再把情绪内核做成 domain 纯函数并接到真实属性与离线结算上。**两步都能跑**。
...
**验收（P1+P0）**：单测全绿；真机——冷启动先睡 → 结算完成按离线结果换脸；时间旅行拨 24h → 属性掉 → 表情转饿/病/闷；喂食/玩耍后表情转瞬态并按时回落；宠物精灵形变与表情同源不打架。
**风险检查点**：R5 双源漂移 —— **本步验证**。

---

### M19 全屏 Robot 互动模式（可跑）

> 目的：完成诉求 ③④⑤——模式切换骨架先跑通（可玩），再接触摸互动与情绪反馈。
...
**验收（P0）**：真机——点顶部表情全屏、宠物到底部中央且大小与顶部表情一致、点它回游走、返回键回游走不退出；ROBOT 模式下宠物不再走动（省电可见）；全屏 Robot 拖拽/点击暂不响应（等 S2）。
...
- [x] `GrokBot` 库内 `pointerInput`（眼随手指）与宿主手势并存、不参与业务判定（**R2 已收口**）：库源码确认它只 `awaitPointerEvent` 记录坐标、**从不 consume**，且 `while(true)` 永不结束（不会触发 `awaitPointerEventScope` 结束时的统一吞事件）。故宿主手势恢复 `requireUnconsumed = true`，只吃干净 down —— 顺带解决「点缩略/点气泡会顺带逗到 Robot」。真机只需确认两者能共存。
- [x] `GreetingBubble` 优先级（原「待真机确认」）：气泡展示期间由它吃掉点击 —— `onVisibleChange` 上报 `bubbleVisible`，Robot 手势层 `enabled` 含 `!bubbleVisible`（双保险：z 更高先消费 + 显式卸责）。
...
**验收（P0）**：真机——全屏内点/滑/长按各有表情与动效反馈并涨心情（冷却内不涨）；边缘三向抽屉手势完全照常；气泡期间不吃交互；回游走后情绪回落正常。
**风险检查点**：R2 手势冲突、R6 手势层重构回归 —— **本步暴露并定案**。

#### M19 之后：M20 打磨（可跑，优先级低）

> D8 已拍板「要做过渡动画，但优先级降低」，故单列打磨里程碑；M18/M19 仍先直切，不影响前两个里程碑的验证节奏。
...
- [x] **M20.S4 圆形舞台：非 1:1 屏锁圆居中 + 整体等比缩放**（用户要求，doc/06 §8.0）：新增 `presentation/component/Stage.kt` → `WatchStage`，由 `BaseActivity.injectContent` 单点接入（`TamagotchiTheme` 之后），**所有 Activity 统一生效**。要点：① 舞台 = `min(可用宽, 可用高)` 见方 + `clip(CircleShape)`，居中，四角露纯黑；② **内部坐标系恒为 `DESIGN_WIDTH`(230dp) 见方，靠替换 `LocalDensity`** —— 不用 `graphicsLayer{scale}`（那只改绘制，触摸坐标不变换，三向抽屉跟手会错位）；③ `scale = 可用短边/230dp` 封顶 `SCALE_MAX`(1.4)，dp 与 sp **一起等比**，fontScale 保留；④ **注入舞台口径 `ScreenMetrics`**（`LocalStageMetrics`），`screenMetrics()` 优先取它 → 短边恒 230、`scale` 恒 1.0，业务公式退化为圆表原值，**DPI 变大只作用在外层那一次缩放**，计算前提不变；⑤ `LocalViewConfiguration` 的 touchSlop / 最小热区同比换算，否则大屏手势过于灵敏。圆表上 scale ≈ 1，与改造前等价（半径/留白实测同值）；`listEdge` 原本按长边一半算（手机上会算出几百 dp 留白）随口径纠正回圆表值。
- [x] **M20.S5 脸型手动覆盖（用户要求：自动保留 + 手动可改）**：新增 `FaceShapeActivity`（独立 Activity，设置页入口「Robot 脸型」，页面框架与其他页一致：`BaseActivity.injectContent` + 圆形舞台 + `LazyColumn` 滚动流 + `RoundEdgeSpace` 首末留白 + `roundEdgeFade`）。列表 = 首项「自动（当前：X）」+ 全部 18 个形状（**圆润系排前**），每行左侧为该形状**真实预览**（`RobotFace` `paused=true` 静态帧 —— 库只在第 0 帧自增 frame，18 项同时挂树上不吃 CPU），名称走 strings（`RobotShapePicker.labelRes`，中英双语）。持久化：偏好 SP `face_shape_<petId>`（**按宠物存**，换宠不继承），选「自动」= 删键；**未知值解析为「跟随自动」而非 BLOB**（否则再也回不到自动）。生效：`EntryFlow` 在 `ON_RESUME` 重读（主屏常驻，不重读看不到刚选的）→ `PetScreen(robotShapeOverride)` → `remember(petId, seed, override) { override ?: pick(...) }`；换形状走库自带 morph 不硬跳。自动选择逻辑（`RobotShapePicker.pick`）**未改**。单测 `SettingsStoreTest` 7 例全绿（新增：按宠隔离 + 清除回自动、未知值=跟随自动、可选清单覆盖全部 18 个且不重复）。
